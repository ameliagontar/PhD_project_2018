function [bestacc,bestF,bestconfus,accvec] = Exp1F2Skel1
...
NC = 5;
TextonArray3D = TextonKmeansSkel1(NC);
...
Hmat = AllTextonMapsSkel1(AllTextons2D);
...
for k1 = 1:NT-1
    for k2 = k1+1:NT;
        kcnt = kcnt + 1;
        Fvec = [k1 k2];
        [confusmat,acc] = LDAclassify9skel1(Hmat,Cohortmat,Fvec);
        accvec(kcnt) = acc;
        if acc > bestacc
